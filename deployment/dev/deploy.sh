...
if [ -z "$IS_BLUE" ]; then
  ...
  echo "2. BLUE 컨테이너 실행"
  docker compose up -p atcha -d atcha-blue --scale atcha-blue=2
  ...
  echo "4. nginx 재실행"
  ensure_nginx_running
  sudo cp -f /root/data/nginx/nginx-blue.conf /root/data/nginx/nginx.conf
  restart_nginx
  ...
  docker compose up -p atcha -d atcha-green --scale atcha-green=2
  ...
  echo "4. nginx 재실행"
  ensure_nginx_running
  sudo cp -f /root/data/nginx/nginx-green.conf /root/data/nginx/nginx.conf
  restart_nginx
