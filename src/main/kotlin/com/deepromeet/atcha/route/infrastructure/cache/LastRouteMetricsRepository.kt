...
class LastRouteMetricsRepository(
    ...
    fun incrTotal(count: Long = 1) {
        val key = "metrics:last_route:${today()}:total"
        stringRedisTemplate.opsForValue().increment(key, count)
        stringRedisTemplate.expireIfNotSet(key)
        stringRedisTemplate.opsForValue().increment("metrics:last_route:total", count)
    }

    fun incrSuccess(count: Long = 1) {
        val key = "metrics:last_route:${today()}:success"
        stringRedisTemplate.opsForValue().increment(key, count)
        stringRedisTemplate.expireIfNotSet(key)
        stringRedisTemplate.opsForValue().increment("metrics:last_route:success", count)
    }

    fun getTodayRate(): Double {
        val total =
            stringRedisTemplate.opsForValue()
                .get("metrics:last_route:${today()}:total")?.toDouble() ?: 0.0
        val success =
            stringRedisTemplate.opsForValue()
                .get("metrics:last_route:${today()}:success")?.toDouble() ?: 0.0
